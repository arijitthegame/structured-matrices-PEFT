    ...
    def init_parameters(self):
        ...
        with torch.no_grad() :
          if self.use_prod :
              ...
              nn.init.zeros_(self.toeplitz_A) # this will likely not work but I do not of a good initialization
...
class ToeplitzLayer_general(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        if self.use_prod :
          x = self.activation_fn((toeplitz_multiplication(self.toeplitz_B , self.toeplitz_B, \
                                  (toeplitz_multiplication(self.toeplitz_A , self.toeplitz_A, self.toeplitz_dropout(x))))) \
                                  + self.toeplitz_bias
        else :
...
class CustomAdapter(nn.Module):
  def __init__(
        ...
        **kwargs
        ...
        seq_list.append(ToeplitzLayer(self.input_size, toeplitz_dropout=self.dropout, activation=self.activation, use_prod=self.use_prod))
