...
class Linear(nn.Linear, CirculantLayer):
    ...
    def __init__(
        ...
        in_features: int,
        out_features: int,
        circ_alpha: float = 1.0,
        circulant_dropout: float = 0.0,
    ...
    def init_parameters(self):
        ...
        if hasattr(self, "circulant_B"):
            ...
            nn.init.normal_(
                self.circulant_A
            )  # bert style initialization mean =0, std = .02
            ...
            if self.merge_weights and self.merged:
                ...
                if self.use_circ:
                    if self.use_prod:
                        ...
                        self.weight.data -= (
                            T(circulant(self.circulant_A).transpose(0, 1))
                            * self.scaling
                        )
                self.merged = False
        else:
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.use_circ and not self.merged:
            ...
            if self.use_prod:
                result += (
                    circulant_multiply(
                        self.circulant_B * self.circulant_A, self.circulant_dropout(x)
                    )
                ) * self.scaling
            else:
                result += (
...
class CirculantBertAttention(BertAttention):
    def __init__(
        ...
        super().__init__(config)
        self.self = CirculantBertSelfAttention(
            config,
...
class CirculantBertLayer(BertLayer):
    def __init__(
        ...
        super().__init__(config)
        self.attention = CirculantBertAttention(
            config,
            apply_circ_q=apply_circ_q,
            apply_circ_k=apply_circ_k,
            apply_circ_v=apply_circ_v,
...
class CirculantBertModel(BertModel):
    def __init__(
        ...
        super().__init__(config)
        self.config = config
        self.encoder = CirculantBertEncoder(
            config,
            apply_circ_q=apply_circ_q,
            apply_circ_k=apply_circ_k,
            apply_circ_v=apply_circ_v,
