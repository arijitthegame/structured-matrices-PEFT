        ...
        if mode:
            if self.merge_weights and self.merged:
                ...
                if self.use_toep:
                    self.weight.data -= (
                        T(
                            toeplitz(self.toeplitz_init_B_r, self.toeplitz_init_B_r)
                            @ toeplitz(self.toeplitz_init_A_r, self.toeplitz_init_A_r)
                        )
                        * self.scaling
                    )
                self.merged = False
        ...
        if self.use_toep and not self.merged:
            ...
            result += (
                toeplitz_multiplication(
                    self.toeplitz_init_B_r,
                    self.toeplitz_init_B_r,
                    toeplitz_multiplication(
                        self.toeplitz_init_A_r,
                        self.toeplitz_init_A_r,
        ...
        **kwargs
        ...
        if self.apply_toep_q:
            self.query = Linear(
                config.hidden_size,
                self.all_head_size,
                toep_alpha=self.toep_alpha,
                trainable_alpha=self.trainable_alpha,
                use_toep=self.use_toep,
                toeplitz_dropout=self.toeplitz_dropout,
                **kwargs,
            )
        else:
...
class ToeplitzBertAttention(BertAttention):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(config)
        self.self = ToeplitzBertSelfAttention(
            config,
            apply_toep_q=apply_toep_q,
            apply_toep_k=apply_toep_k,
            apply_toep_v=apply_toep_v,
            toep_alpha=toep_alpha,
            trainable_alpha=trainable_alpha,
            use_toep=use_toep,
            toeplitz_dropout=toeplitz_dropout,
...
class ToeplitzBertLayer(BertLayer):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(config)
        self.attention = ToeplitzBertAttention(
            config,
            apply_toep_q=apply_toep_q,
            apply_toep_k=apply_toep_k,
            apply_toep_v=apply_toep_v,
...
class ToeplitzBertModel(BertModel):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(config)
        self.config = config
        self.encoder = ToeplitzBertEncoder(
            config,
            apply_toep_q=apply_toep_q,
            apply_toep_k=apply_toep_k,
            apply_toep_v=apply_toep_v,
