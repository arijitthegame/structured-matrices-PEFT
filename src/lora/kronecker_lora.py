...
class Linear(nn.Linear, KroneckerLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        KroneckerLayer.__init__(self, r=r, kronecker_dropout=kronecker_dropout,
                           merge_weights=merge_weights, tie_weights=tie_weights)
        ...
        if r > 0:
            ...
            self.kronecker_A = nn.Parameter(self.weight.new_zeros((r, int(in_features/r))))
            self.kronecker_B = nn.Parameter(self.weight.new_zeros((int(out_features/r), r)))
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, 'kronecker_A'):
            ...
            if self.tie_weights is False :
              ...
              nn.init.zeros_(self.kronecker_B)
              nn.init.zeros_(self.kronecker_A) # this will likely not work but I do not of a good initialization
    ...
    def train(self, mode: bool = True):
        ...
        if mode:
            if self.merge_weights and self.merged:
                ...
                if self.r > 0:
                    self.weight.data -= T(torch.kron(self.kronecker_B, self.kronecker_A)) * self.scaling
                self.merged = False
        else:
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.r > 0 and not self.merged:
            ...
            result += (self.kronecker_dropout(x) @ (torch.kron(self.kronecker_B, self.kronecker_A)).transpose(0,1)) * self.scaling
            return result
        else:
...
class MergedLinear(nn.Linear, KroneckerLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        KroneckerLayer.__init__(self, r=r, kronecker_dropout=kronecker_dropout,
                           merge_weights=merge_weights, tie_weights=tie_weights)
        assert out_features % len(enable_kronecker) == 0, \
        ...
        if r > 0 and any(enable_kronecker):
            self.kronecker_A = self.weight.new_zeros((r * sum(enable_kronecker), int(in_features/r)))
            self.kronecker_B = self.weight.new_zeros(((out_features // len(enable_kronecker) * sum(enable_kronecker))//r), r)

            self.kronecker_A = nn.Parameter(self.kronecker_A.reshape(sum(self.enable_kronecker), self.r, int(self.in_features/self.r)))
            self.kronecker_B = nn.Parameter(self.kronecker_B.reshape(sum(self.enable_kronecker), -1, self.r))
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, 'kronecker_A'):
            ...
            if self.tie_weights is False :
              ...
              nn.init.zeros_(self.kronecker_A)  # this will likely not work. I do not know of a proper initialization scheme
              nn.init.zeros_(self.kronecker_B)
    ...
    def merge_AB(self):
        ...
        delta_w = batch_kron(self.kronecker_A, self.kronecker_B).reshape(-1, self.in_features)
