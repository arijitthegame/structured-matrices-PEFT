  ...
  """
    ...
    if bias == 'none':
        ...
        for m in model.modules():
            if isinstance(m, CirculantLayer) and \
                hasattr(m, 'bias') and \
                m.bias is not None:
...
def circulant_state_dict(model: nn.Module, bias: str = 'none') -> Dict[str, torch.Tensor]:
    """
    ...
    if bias == 'none':
        ...
        return {k: my_state_dict[k] for k in my_state_dict if 'circulant_' in k.lower() or 'bias' in k}
    elif bias == 'circulant_only':
        to_return = {}
...
def mark_only_kronecker_as_trainable(model: nn.Module, bias: str = 'none') -> None:
    ...
    if bias == 'none':
        ...
        for m in model.modules():
            if isinstance(m, KroneckerLayer) and \
                hasattr(m, 'bias') and \
                m.bias is not None:
                    m.bias.requires_grad = True
    else:
...
def kronecker_state_dict(model: nn.Module, bias: str = 'none') -> Dict[str, torch.Tensor]:
    ...
    if bias == 'none':
        ...
        return {k: my_state_dict[k] for k in my_state_dict if 'kronecker_' in k.lower() or 'bias' in k}
    elif bias == 'kronecker_only':
        to_return = {}
        for k in my_state_dict:
            if 'kronecker_' in k:
...
def mark_only_toeplitz_as_trainable(model: nn.Module, bias: str = 'none') -> None:
    ...
    if bias == 'none':
        ...
        for m in model.modules():
            if isinstance(m, ToeplitzLayer) and \
                hasattr(m, 'bias') and \
                m.bias is not None:
                    m.bias.requires_grad = True
    else:
...
def toeplitz_state_dict(model: nn.Module, bias: str = 'none') -> Dict[str, torch.Tensor]:
    """Function to save the Toeplitz parameters"""
    my_state_dict = model.state_dict()
    if bias == 'none':
        return {k: my_state_dict[k] for k in my_state_dict if 'toeplitz_' in k.lower()}
    elif bias == 'all':
        return {k: my_state_dict[k] for k in my_state_dict if 'toeplitz_' in k.lower() or 'bias' in k}
    elif bias == 'toeplitz_only':
        to_return = {}
        for k in my_state_dict:
            if 'toeplitz_' in k:
